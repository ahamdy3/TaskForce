...
class LeaderJobsImpl(config: LeaderJobsConfig, leaderDuties: LeaderDuties) extends LeaderJobs with Logging {
  ...
  override val scheduler: Scheduler = new SchedulerImpl(SchedulerConfig(threadPoolSize = 5))
...
case class LeaderJobsConfig(leaderElectionPeriod: FiniteDuration, refreshQueuedJobsPeriod: FiniteDuration,
                            assignQueuedJobsPeriod: FiniteDuration, cleanJobsPeriod: FiniteDuration,
                            scaleClusterPeriod: FiniteDuration)
